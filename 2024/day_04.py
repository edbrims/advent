...
def letter_is_at_by_vector(letter, lines, x_row, x_col, unit_vector, multiplier):
    vector = [x * multiplier for x in unit_vector]
    row = x_row + vector[0]
    col = x_col + vector[1]
...
def count_from_x(lines, x_row, x_col):
    ...
    for vector in vectors:
        if (letter_is_at_by_vector('M', lines, x_row, x_col, vector, 1) and letter_is_at_by_vector('A', lines, x_row, x_col, vector, 2) and letter_is_at_by_vector('S', lines, x_row, x_col, vector, 3)):
            count += 1
    return count
...
def crossing_around_a(lines, r, c):
    ...
    corners = f'{lines[r-1][c-1]}{lines[r+1][c-1]}{lines[r+1][c+1]}{lines[r-1][c+1]}'
